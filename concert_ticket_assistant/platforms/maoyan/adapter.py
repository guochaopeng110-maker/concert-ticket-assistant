...
class MaoyanAdapter:
    ...
    def _fetch_payload(self, event_id: str, session_id: str) -> dict[str, Any]:
        ...
        if self.session is not None:
            try:
                ...
                raise MaoyanAdapterError("Maoyan request failed", AdapterErrorKind.NETWORK) from exc
    ...
    def _parse_payload(payload: str) -> dict[str, Any]:
        ...
        lower = body.lower()
        if not body:
            raise MaoyanAdapterError("Empty Maoyan payload", AdapterErrorKind.TEMPORARY_UNAVAILABLE, body)
        if body.startswith("<"):
            if "login" in lower:
                raise MaoyanAdapterError("Maoyan login required", AdapterErrorKind.NOT_LOGGED_IN, body)
            if any(x in body for x in ("验证码", "安全验证")) or "anti" in lower:
                raise MaoyanAdapterError("Maoyan risk control page", AdapterErrorKind.RISK_CONTROL, body)
            raise MaoyanAdapterError("Maoyan API changed: HTML response", AdapterErrorKind.API_CHANGED, body)
        if any(x in body for x in ("系统繁忙", "稍后再试")):
            raise MaoyanAdapterError("Maoyan temporary payload", AdapterErrorKind.TEMPORARY_UNAVAILABLE, body)
        try:
            data = json.loads(body)
        except json.JSONDecodeError as exc:
            raise MaoyanAdapterError("Failed to parse Maoyan payload", AdapterErrorKind.PARSE_ERROR, body) from exc
        if not isinstance(data, dict):
            raise MaoyanAdapterError("Maoyan payload is not an object", AdapterErrorKind.API_CHANGED, body)
        if "data" not in data:
            raise MaoyanAdapterError("Maoyan payload missing required fields", AdapterErrorKind.API_CHANGED, body)
        return data
